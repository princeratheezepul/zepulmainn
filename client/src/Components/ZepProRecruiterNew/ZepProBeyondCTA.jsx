...
const ZepProBeyondCTA = () => {
  useEffect(() => { const revealObserver = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('visible'); revealObserver.unobserve(entry.target); } }); }, { threshold: 0.08, rootMargin: '0px 0px -30px 0px' }); const rootNode = document.getElementById('ZepProBeyondCTA-root'); if (rootNode) { rootNode.querySelectorAll('.reveal').forEach(el => revealObserver.observe(el)); } return () => revealObserver.disconnect(); }, []);
  return (
    <div id="ZepProBeyondCTA-root" className="zep-pro-recruiter-page">
  ...
  <div className="beyond-left reveal">
    <div>
      ...
      <p style={{fontSize: '13px', lineHeight: '1.75', color: 'rgba(255,255,255,.45)', marginTop: '16px', maxWidth: '420px'}}>
    ...
    <form onSubmit="event.preventDefault()">
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          ...
          <select><option>Select range</option><option>1—10 hires/month</option><option>11—50 hires/month</option><option>50+ hires/month</option></select>
        </div>
      </div>
...
export default ZepProBeyondCTA;
