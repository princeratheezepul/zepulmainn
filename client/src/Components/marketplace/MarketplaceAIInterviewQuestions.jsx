...
const MarketplaceAIInterviewQuestions = ({ onBack, jobDetails, resumeData }) => {
  ...
  const fetchAIQuestions = async () => {
    ...
    try {
        ...
        const prompt = `
            ...
            Candidate's Key Skills: ${resumeData.skills ? resumeData.skills.join(", ") : 'Not specified'}
            Candidate's Experience: ${resumeData.aiSummary?.technicalExperience || resumeData.experience || 'Not specified'}
  ...
  const handleScheduleInterview = async () => {
    const selectedSchedule = schedule.find(s => s.date.toISOString() === selectedDate);
    if (!selectedSchedule) {
    ...
    setScheduling(true);
    try {
      // For marketplace, we'll simulate the scheduling since we don't have the same backend endpoint
      setInterviewScheduled(true);
      setSelectedScheduleData({
...
export default MarketplaceAIInterviewQuestions;