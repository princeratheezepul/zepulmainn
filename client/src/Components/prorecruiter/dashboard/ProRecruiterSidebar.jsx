...
const ProRecruiterSidebar = ({ activeComponent, setActiveComponent, isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <>
      <div
        className={`bg-white h-screen fixed top-0 left-0 z-10 hidden lg:flex flex-col justify-between transition-all duration-300 border-r border-gray-200 ease-in-out ${isCollapsed ? "w-20 items-center" : "w-52"
          }`}
      >
        {/* Header / Logo + Toggle */}
        ...
        <nav className="flex-1 flex-col gap-2 px-2 py-4 w-full">
          {navItems.map((item) => (
            <div
              key={item.name}
              className={`flex items-center w-full px-3 py-2.5 text-sm transition-colors duration-200 mb-1 cursor-pointer ${activeComponent === item.name
        ...
        <div
          className={`flex items-center px-3 py-3 border-gray-200 ${isCollapsed ? "justify-center" : ""
            }`}
        >
          <img
...
export default ProRecruiterSidebar;
