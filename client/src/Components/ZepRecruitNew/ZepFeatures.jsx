...
const ZepFeatures = () => {
  useEffect(() => { const revealObserver = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('visible'); revealObserver.unobserve(entry.target); } }); }, { threshold: 0.08, rootMargin: '0px 0px -30px 0px' }); const rootNode = document.getElementById('ZepFeatures-root'); if (rootNode) { rootNode.querySelectorAll('.reveal').forEach(el => revealObserver.observe(el)); } return () => revealObserver.disconnect(); }, []);
  return (
    <div id="ZepFeatures-root" className="zep-recruit-page">
      <section className="zrw-features">
  <div className="zrw-features-header reveal">
    <h2 className="section-head">What Zep Recruit<br /><span className="blue">Delivers For You</span></h2>
  </div>
  <div className="zrw-feature-grid">
    ...
    <div className="zrw-feature-card reveal">
      ...
      <div className="zrw-feature-title">Plan &amp; Deploy</div>
      <div className="zrw-feature-desc">Demand forecasting with a dedicated recruitment team aligned to your hiring goals and timelines.</div>
    </div>
    {/*  2  */}
    <div className="zrw-feature-card reveal">
