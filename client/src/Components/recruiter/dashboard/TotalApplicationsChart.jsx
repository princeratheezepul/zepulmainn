...
const TotalApplicationsChart = () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (errorText.includes("<!DOCTYPE") || errorText.includes("<html")) {
            throw new Error(
              `Server returned HTML instead of JSON. This might be an authentication issue. Status: ${response.status}`,
            )
          }
  ...
  return (
    <div className="bg-white rounded-lg p-2 shadow-md font-sans h-full">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 h-[calc(100%-24px)]">
        <div className="relative w-full sm:w-1/2 h-48 sm:h-full min-h-[180px]">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
            <div className="text-2xl sm:text-3xl font-bold leading-none">{chartTotal}</div>
            <div className="text-gray-500 text-xs sm:text-sm">Total Applications</div>
          </div>
        </div>
          ...
          <ul className="list-none p-0 m-0 flex flex-col gap-1 md:gap-2">
            {legendData.map((entry, index) => (
              <li key={`item-${index}`} className="flex items-center gap-2">
                ...
                <span className="bg-gray-800 text-white py-0.5 px-1.5 rounded-xl text-[10px] font-medium min-w-[24px] text-center flex-shrink-0">
                  {entry.value}
                </span>
              </li>
            ))}
