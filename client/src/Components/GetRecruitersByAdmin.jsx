...
const GetRecruitersByAdmin = () => {
    ...
    const fetchRecruiters = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/recruiter/getrecruiter?creatorId=${creatorId}&role=${role}`
        );
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-4">Recruiters Under You (Admin)</h2>
      {recruiters.length === 0 ? (
...
export default GetRecruitersByAdmin;
