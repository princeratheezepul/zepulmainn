...
export default function CandidateLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${config.backendUrl}/api/candidate/login`, {
        method: "POST",
      ...
      setTimeout(() => navigate("/candidate/dashboard", { replace: true }), 600);
    } catch (error) {
    ...
    <div className="relative min-h-screen flex flex-col md:flex-row bg-white">
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 min-h-screen px-4 py-8 md:py-0">
        <div className="w-full max-w-xl p-0 md:p-0 rounded-xl">
          ...
          <p className="text-gray-500 mb-8 text-sm">Sign in to view jobs and track your applications.</p>
          ...
          <div className="mt-6 text-center text-sm text-gray-500">
            ...
            <Link to="/candidate/signup" className="text-blue-600 font-medium hover:underline">Sign up</Link>
          </div>
        </div>
      </div>
    </div>
