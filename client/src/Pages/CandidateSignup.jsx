...
export default function CandidateSignup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${config.backendUrl}/api/candidate/signup`, {
        method: "POST",
      ...
      // Persist candidate session so the details + dashboard pages can use it
      if (data.data?.accessToken) {
        localStorage.setItem("candidateToken", data.data.accessToken);
      }
      localStorage.setItem("candidateInfo", JSON.stringify(data.data?.user || {}));

      toast.success("Account created! Let's complete your profile.");
      setTimeout(() => navigate("/candidate/details"), 800);
    } catch (error) {
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-white">
      ...
      <div className="flex flex-col justify-center items-center w-full md:w-1/2 min-h-screen px-4 py-8 md:py-0">
        <div className="w-full max-w-xl p-0 md:p-0 rounded-xl">
          <h2 className="text-3xl font-semibold mb-2">Create your candidate account</h2>
          <p className="text-gray-500 mb-8 text-sm">Find your next role with Zepul.</p>
